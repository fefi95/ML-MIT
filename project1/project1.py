...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    return 1 - agreement if agreement < 1 else 0
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    """
    loss = [
        hinge_loss_single(x, labels[i], theta, theta_0)
...
def perceptron_single_step_update(
    ...
    zero_one_loss = 1 if agreement > 0 else 0
...
def average_perceptron(feature_matrix, labels, T):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            list_of_thetas_0.append(theta_0)

    return (
        sum(list_of_thetas) / len(list_of_thetas),
        sum(list_of_thetas_0) / len(list_of_thetas_0),
    )
...
def classify(feature_matrix, theta, theta_0):
    ...
    return np.array(
        [
            1 if np.dot(theta, feature_vector) + theta_0 > 0 else -1
...
def classifier_accuracy(
    ...
    **kwargs
    ...
    for i in range(max_iter):
        ...
        current_cost = theta_0

        if abs(current_cost - cost) <= 0.001:
            break  # Convergence
        cost = current_cost

    accuracy_train_data = accuracy(
        classify(train_feature_matrix, theta, theta_0), train_labels
    )
    accuracy_val_data = accuracy(
        classify(val_feature_matrix, theta, theta_0), val_labels
    )

    return (accuracy_train_data, accuracy_val_data)
